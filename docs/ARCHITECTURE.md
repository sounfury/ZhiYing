...
| 日期 | 2026-07-15 |
| 依据 | [PRD v0.2](./PRD.md) |
| 状态 | 当前实现参考；关系后处理 V2 正在按根目录 `重构方案.md` 重构 |
| 部署假设 | **本机个人使用**；默认信任操作者，不为「防恶意入参 / 多租户」提前付工程税 |

> **当前实现（2026-09-22）：** 已经在跑的分析流水线，包括哪一步调用哪个 Agent、哪一步只是纯逻辑，见 [human/分析流水线.md](./human/分析流水线.md)。本文其余章节是 2026-07 的设计稿，和代码不一致时以该文为准。
>
> **V2 提示（2026-09-12）：** 关系后处理的目标方案在 [design/重构方案.md](./design/重构方案.md)。Phase 0 / 1 已完成，Phase 2 起尚未实施。V2 落地后应整体同步本文件，而不是继续追加平行设计稿。
