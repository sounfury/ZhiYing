...
> **V2 提示（2026-09-12）：** 本文中关于固定 `Reconcile Agent`、关系短枚举、按章关系归一化、旧 `relation_overrides` / `type_filter` 的章节描述的是 V1/当前实现，不再代表目标架构。当前目标以 [`../重构方案.md`](../重构方案.md) 为准；V2 落地后应整体同步本文件，而不是继续追加平行设计稿。
